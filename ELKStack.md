...
이 프로젝트는 **이상치 거래 탐지 시스템**을 구축하여 금융 및 결제 데이터에서 발생하는 **비정상적인 소비 패턴**을 실시간으로 탐지하고, **보험사기**, **자금세탁**, **허위 청구** 등의 고위험 거래를 예방하는 시스템을 개발하는 것입니다. 이를 기반으로 **금융당국**과 협력하여 **위험 요소를 사전에 차단**하는 것을 목표로 합니다. **Kibana**를 사용하여 데이터를 시각화하고, 이상 거래를 모니터링합니다.
...
- **Y축 (세로)** : 보험비 금액  
  선택한 고객들의 **보험비 사용 금액**을 기준으로, 평균과 최대를 각각 표시합니다.

---

### ✅ 집계 방식
- **🟢 보험비 평균값** : 각 고객별 보험비 사용 금액의 평균
- **🟣 보험비 최대값** : 각 고객별 보험비 사용 금액 중 가장 큰 값
...
- 보라색(최대값)이 초록색(평균값)보다 훨씬 큰 피크가 발생하는 구간은  
...
- 특히 최대값(🔵)이 700 이상으로 치솟는 구간은, **특정 고객이 고위험 업종에 매우 높은 금액을 집중 사용**했음을 보여줍니다.
